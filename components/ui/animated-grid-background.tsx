...
function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    if (!vs) { drawError(gl, `Vertex compile error: ${vsLog}`); return cleanup; }
    const { shader: fs, log: fsLog } = safeCompile(gl, gl.FRAGMENT_SHADER, fragSource);
    if (!fs) { drawError(gl, `Fragment compile error: ${fsLog}`); gl.deleteShader(vs); return cleanup; }
    const linked = safeLink(gl, vs, fs);
    gl.deleteShader(vs); gl.deleteShader(fs);
    if (!linked.program) { drawError(gl, `Program link error: ${linked.log}`); return cleanup; }
...
/* ========= Background wrapper component for sections ========= */
